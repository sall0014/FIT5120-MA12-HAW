...
if(!is_plugin_active( 'affiliates-manager/boot-strap.php')){return;}
		...
		**/
		public static function _after_added_wpam_transaction($data=array())
		{
			...
			if(sizeof($am_user)>0)
			{
				...
				if($buyer_sms_notify=='on' && $buyer_sms_content!='')
				{
					$wpam_user				=array();
				...
				if($admin_sms_notify=='on' && $admin_phone_number!='' && $admin_sms_content!='')
				{
		...
		public static function addTabs($tabs=array())
		{
			...
			$tabs['affiliate_manager']['inner_nav']['wpam_customer']['tabContent']  	= self::getContentFromTemplate('views/message-template.php',$customer_param);
			...
			$tabs['affiliate_manager']['inner_nav']['wpam_admin']['tabContent']	= self::getContentFromTemplate('views/message-template.php',$admin_param);
			return $tabs;
		}
		...
		public static function getCustomerTemplates()
		{
			...
			foreach($wpam_statuses as $ks  => $vs){
				...
				$checkboxNameId	= 'smsalert_wpam_general[wpam_order_status_'.$vs.']';
				$textareaNameId	= 'smsalert_wpam_message[wpam_sms_body_'.$vs.']';
		...
		public static function getAdminTemplates()
		{
			...
			foreach($wpam_statuses as $ks  => $vs){
				...
				$checkboxNameId	= 'smsalert_wpam_general[wpam_admin_notification_'.$vs.']';
				$textareaNameId	= 'smsalert_wpam_message[wpam_admin_sms_body_'.$vs.']';
