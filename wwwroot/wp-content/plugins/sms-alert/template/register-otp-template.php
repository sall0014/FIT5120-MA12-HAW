...
if(!headers_sent())
							...
							<div class="sa_customer_validation-modal-dialog sa_customer_validation-modal-md">
								<div class="login sa_customer_validation-modal-content">
									<div class="sa_customer_validation-modal-header">
										<b>'.__("Validate OTP (One Time Passcode)","sms-alert").'</b>
		...
		echo'								<div class="mo_customer_validation-login-container">
												<form name="f" method="post" action="">
													...
													<input type="number" name="smsalert_customer_validation_otp_token"  autofocus="true" placeholder="" id="smsalert_customer_validation_otp_token" required="true" class="sa_customer_validation-textbox" autofocus="true" pattern="[0-9]{4,8}" title="'.SmsAlertMessages::showMessage('OTP_RANGE').'" />
													<br /><input type="submit" name="smsalert_otp_token_submit" id="smsalert_otp_token_submit" class="smsalert_otp_token_submit"  value="'.__("Validate OTP","sms-alert").'" />
													<input type="hidden" name="otp_type" value="'.$otp_type.'">';
													if(!$from_both){
		echo'											<input type="hidden" id="from_both" name="from_both" value="false" />
														<a style="float:right" id="verify_otp" onclick="sa_otp_verification_resend();">'.SmsAlertMessages::showMessage('RESEND_OTP').'</a>
		...
		echo'											<input type="hidden" id="from_both" name="from_both" value="true" />
														<a style="float:right" id="verify_otp" onclick="sa_select_goback();">'.SmsAlertMessages::showMessage('RESEND_OTP').'</a>
														<span id="timer" style="min-width:80px; float:right">00:00 sec</span>';
													}
